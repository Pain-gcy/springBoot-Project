...
-XX:+PrintGC -Xloggc:/data1/logs/apollo/apollo-charging-sncmcc-zx-query/gc.log"

pid=`ps -eo pid,args | grep apollo-charging-sncmcc-zx-query | grep java | grep -v grep | awk '{print $1}'`
if [ -n "$pid" ]
then
    kill -3 ${pid}
    kill ${pid} && sleep 3
